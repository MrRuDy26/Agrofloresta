...
// Importações cruciais - Verifique se os caminhos batem com suas pastas
    ...
    <div className="min-h-screen bg-stone-50 font-sans text-stone-900">
      <nav className="bg-white border-b p-4 flex justify-between items-center sticky top-0 z-50 px-6">
        ...
        <div className="flex items-center gap-4">
          {user ? (
            ...
            <button onClick={handleGoogleLogin} className="text-xs font-bold bg-stone-100 px-4 py-2 rounded-xl border border-stone-200">
              ENTRAR
            </button>
          )}
          <button onClick={() => setStep('premium')} className="text-[10px] font-black bg-amber-100 text-amber-700 px-3 py-1 rounded-full">
              🔒 PREMIUM
          </button>
        </div>
      </nav>
      ...
      {step === 'hero' && (
        <div className="py-32 px-4 text-center bg-stone-900 text-white">
          ...
          <button onClick={() => setStep('form')} className="px-10 py-5 bg-green-600 rounded-full font-bold text-xl hover:bg-green-500 transition-all">
            COMEÇAR
          </button>
        </div>
      )}
